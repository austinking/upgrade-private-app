...
function install(appDirPath) {
    ...
    which.on('close', function(code) {
        ...
        moveApp(appDirPath, destination);
    });
}

function moveApp (appDirPath, destination) {
    ...
    mv.stderr.on('error', function(err) {
        process.stdout.write('Unable to move ' + appDirPath + ' to ' +
            destination + ' err:' + err.toString() + '\n');
    });
